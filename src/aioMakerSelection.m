...
if (exist('imName','var'))
    imgNum=find(contains(imagesList,imName));
end
...
for idxSmoothingMethod=1:length(Experiment.smoothingMethod)
    ...
    for idxFunc = 1:size(Experiment.config.FwMeasure,2)

        choquetType = Experiment.config.FwMeasure{idxFunc}{1};
        F1 = Experiment.config.FwMeasure{idxFunc}{2};
        F2 = Experiment.config.FwMeasure{idxFunc}{3};
        measureType = [Experiment.config.FwMeasure{idxFunc}{4} '-' sigma2name(Experiment.config.FwMeasure{idxFunc}{5})];
        p = Experiment.config.FwMeasure{idxFunc}{5};

        mW = powerMeasure(Experiment.params.measure.tam,p)';
            ...
            if (~exist([Experiment.bdryPublishDir rawImageName],'dir'))
                mkdir([Experiment.bdryPublishDir rawImageName]);
            end

            if (~exist([Experiment.featImDirColor rawImageName],'dir'))
                mkdir([Experiment.featImDirColor rawImageName]);
            end
            ...
            if ((~exist(cpDataFilePath,'file')) || (Experiment.forceCpMaker==1))
                ...
                ftImgColorFilePath = sprintf('%s%s/%s_color.%s',Experiment.featImDirColor,rawImageName,ftFileName,Experiment.imageExt);
                ...
                if ((~exist(ftImgFilePath,'file')) || (Experiment.forceFtMaker==1) )
                    ...
                    if (Experiment.FtWriter)
                       ...
                       writeColorMap(imgFeat.^.65,ftImgColorFilePath);
                    end
                ...
                bdryImgPublishFilePath = sprintf('%s%s/%s.%s',Experiment.bdryPublishDir,rawImageName,bdryFileName,Experiment.imageExt);
                ...
                if ((~exist(bdryImgFilePath,'file')) || (Experiment.forceBdryMaker==1) )
                    ...
                    if (Experiment.BdryWriter)
                       ...
                       imwrite(255-imgBdry,bdryImgPublishFilePath,'png');
                    end
                else
